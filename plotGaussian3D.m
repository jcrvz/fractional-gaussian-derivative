...
% time series
time_nod    = 100;
time        = linspace(1,10,time_nod);

% nu values
nu_nod      = 500;
n           = 6;
nu          = linspace(0.01,0.99 + n - 1,nu_nod);
...
% plotting
imagesize            = [0.95 0.95];
papersize           = [40 40]; % in centimeters
...
Data = DG./repmat(max(abs(DG),[],2),1,time_nod);

surf(T,N,Data), %shading interp, colormap hot,
...
set(gca,'LineWidth',graph.LineWidth,'FontSize',graph.FontSize,'FontName',...
    ...
    'TickLabelInterpreter', 'latex','Units','Normalized','Position',[0.1 0.15 0.85 0.8]),
xlabel('$$t$$ (dimensionless)','FontSize',graph.FontSize,'FontName',graph.FontName,...
    'Interpreter','latex'); box on,
ylabel('$$n+\nu$$ (dimensionless)','FontSize',graph.FontSize,...
    'FontName',graph.FontName,'Interpreter','latex');
%ylabel('\nu-order','FontSize',graph.FontSize,'FontName',graph.FontName);
zlabel('Normalised $$D^{n+\nu}_t G_{\mu,\sigma}(t)$$','FontSize',graph.FontSize,...
    'FontName',graph.FontName,'Interpreter','latex');
view(2);%[-130,30]),

 title('Normalised $$D^{n+\nu}_t G_{\mu,\sigma}(t)$$','FontSize',graph.FontSize,'FontName',graph.FontName,'Interpreter','latex');
 set(colorbar,'TickLabelInterpreter','latex')
 print(gcf, '-dpng','-r150', 'fig2.pdf');